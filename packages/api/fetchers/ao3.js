...
class Ao3Fetcher {
  ...
  async update() {
    ...
    const postsForCache = ao3Posts.map((post) => ({
      ...post,
      id: slugify(`${post.source} ${post.title}`),
    }));
  ...
  async parsePosts(htmlString) {
    ...
    const isPaginaged =
      document.querySelector("ol.pagination.actions") !== null;
    const isFirstFetch = this.cache.length === 0;

    if (isFirstFetch && isPaginaged) {
      // # of children of the pagination element
      const childCount = document.querySelector("ol.pagination.actions")
        .childElementCount;
      const lastPageLink = document.querySelector("ol.pagination.actions")
